...
params = {"Number of rows (n)": 15,
          ...
          "Length distribution address": "C:\\Users\\elc20\\OneDrive\\Documents\\University\\Year 4\\Project\\2D Model\\Report things 2 Random length and doping\\exp_length_dist_Si 15x15 ND 1e16 L 15um T 300K NT 4e11 Point Contacts 2.txt",
          "Doping distribution type": "poisson", #poisson or constant
          "ND": 1e16, #if constant
          "Doping distribution address": "C:\\Users\\elc20\\OneDrive\\Documents\\University\\Year 4\\Project\\2D Model\\Report things 2 Random length and doping\\poisson_doping_dist_Si 15x15 ND 1e16 L 15um T 300K NT 4e11 Point Contacts 2.txt", 
          "Applied voltage": 1,
          ".raw file": "C:\\Users\\elc20\\OneDrive\\Documents\\University\\Year 4\\Project\\2D Model\\Report things 2 Random length and doping\\Si 15x15 ND 1e16 L 15um T 300K NT 4e11 Point Contacts 2.raw",
          "Sim Dir": "Report things 2 Random length and doping",
          "Sim file": "Si 15x15 ND 1e16 L 15um T 300K NT 4e11 Point Contacts 2",
          "Voltage divider data?": False,
          }
...
def load_files(n=params["Number of rows (n)"], m=params["Number of columns (m)"], SimDir=params["Sim Dir"], SimFile=params["Sim file"], L_file=params["Length distribution address"], L_dist=params["Length distribution type"], length=params["Length"], ND_const=params['ND'], doping_dist=params["Doping distribution type"], ND_file=params["Doping distribution address"]):
    Imap = np.loadtxt(path_here+'\\'+SimDir+'\\current_map_'+SimFile+'.txt')

    try:
        Vmap = np.loadtxt(path_here+'\\'+SimDir+'\\voltage_map_'+SimFile+'.txt')
    except:
...
def griddata_plot(x, y, z, Vmap, input_counter, output_counter, boundary_counter, ND, Ldist, L=params['Length']):
    ...
    if params['Voltage divider data?']==True:
        ...
        np.savetxt(path_here+'\\'+params['Sim Dir']+'\\x_XS_voltage'+params['Sim file']+'.txt', Xlim)
        np.savetxt(path_here+'\\'+params['Sim Dir']+'\\y_XS_voltage'+params['Sim file']+'.txt', Ylim)
        print(f'VlimMap={VlimMap}')
    else:
        try:
            Xlim = np.loadtxt(path_here+'\\'+params['Sim Dir']+'\\x_XS_voltage'+params['Sim file']+'.txt')
            Ylim = np.loadtxt(path_here+'\\'+params['Sim Dir']+'\\y_XS_voltage'+params['Sim file']+'.txt')
        except:
